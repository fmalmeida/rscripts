...
All resistance genes from this dataset and its protein ID (local protein ID) are shown in Table \@ref(tab:ncbi-amr-resistance-genes). Aditionally, Figure \@ref(fig:ncbi-resistome-png) shows the number of genes annotated that confer resistance to specific drug classes.
...
genes <- genes %>% select("Prokka_ID", "Prokka_product", "db_id", "V2", "V5", "V9")
...
kable(caption = "Resistance genes annotated from NCBI-amr curated database", escape = FALSE, col.names = c("Local Protein ID", "Annotated Gene Product", "Reference Motif", "Gene Family", "Gene Name", "Description")) %>%
...
```{r ncbi-resistome-png, fig.align='center', fig.show='hold', fig.cap="Drug Classes to which the query genome is resistant (NCBI-amr)", out.width="60%"}
ncbi_count <- genes %>%
