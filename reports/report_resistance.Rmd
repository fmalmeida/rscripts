...
params:
  query: GCF_000240185.1_ASM24018v2_genomic
  amrfinder: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/AMRFinderPlus/AMRFinder_resistance-only.tsv
  gff: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/gffs/GCF_000240185.1_ASM24018v2_genomic.gff 
  rgitool: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/RGI/RGI_GCF_000240185.1_ASM24018v2_genomic.txt
  rgiperfect: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/RGI/Perfect_RGI_GCF_000240185.1_ASM24018v2_genomic_hits.txt
  rgistrict: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/RGI/Strict_RGI_GCF_000240185.1_ASM24018v2_genomic_hits.txt
  argminer_blastp: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/ARGMiner/GCF_000240185.1_ASM24018v2_genomic_argminer_blastp_onGenes.txt
  argminer_blastx: /Volumes/falmeida1TB/bacannot_teste/teste/GCF_000240185.1_ASM24018v2_genomic/resistance/ARGMiner/GCF_000240185.1_ASM24018v2_genomic_argminer_blastx_onGenome.summary.txt
output:
...
if (class(amrtsv) != 'try-error' && nrow(amrtsv) > 1) {
...
if (class(rgi_tsv) != 'try-error' && nrow(rgi_tsv) > 1) {
  rgi_not_null <- TRUE
  rgi_null <- FALSE
} else {
  rgi_not_null <- FALSE
...
* [AMRFinderPlus](https://github.com/ncbi/amr/wiki) and [ARGminer](https://bench.cs.vt.edu/argminer/#/home) are searched in a manner that it is possible to filter alignments based on coverage and identity, for those we have used the following:
    * Blast % identity: `> `r params$blast_id`%`
    * Blast % query coverage: `> `r params$blast_cov`%`
        + For blasts against gene sequences predicted by Prokka, alignments are filtered in a two way manner so they cover at least `r params$blast_cov`% of the reference gene sequence and the of query sequence at the same time -- valid for ARGminer
        + For blasts against the whole genome, alignments are filtered in a one way manner so they cover at least `r params$blast_cov`% of only the reference gene sequence -- valid for ARGminer
