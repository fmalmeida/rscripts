...
# Create List 
string.list <- rgi_perfect %>% mutate_if(is.factor, as.character) %>% separate_rows(5, sep = "; ") %>% pull(5) %>% sort() %>% unique()
...
# Produce Table
rgi_perfect %>%
  select(Prokka_ID, Product, `AMR Gene Family`, 6, 5, CVTERM.ID, `Query Protein Coordinates`) %>%
...
rgi_count <- rgi_perfect %>%
  ...
  separate_rows(5, sep = "; ", convert = FALSE) %>%
  select(Prokka_ID, 5)
...
rgi_count <- rgi_perfect %>%
  mutate_if(is.factor, as.character) %>%
  separate_rows(6, sep = "; ", convert = FALSE) %>%
  select(Prokka_ID, 6)
colnames(rgi_count) <- c("Contig", "Class")
drugs <- plyr::count(rgi_count, "Class")
