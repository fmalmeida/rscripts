...
if (class(resfams_summary) != 'try_error' & nrow(resfams_summary) > 0) {
...
if (class(ncbi_amr) != 'try_error' & nrow(ncbi_amr) > 0) {
  ncbi_not_null <- TRUE
  ncbi_null <- FALSE
} else {
  ncbi_not_null <- FALSE
...
if (class(card_summary) != 'try_error' & nrow(card_summary) > 0) {
  card_not_null <- TRUE
  card_null <- FALSE
} else {
  card_not_null <- FALSE
...
[NCBI-amr](https://ftp.ncbi.nlm.nih.gov/hmm/NCBIfam-AMR/latest/) is the NCBI's Bacterial Antimicrobial Resistance Reference Gene Database that is used by their tool [AMRFinder](https://www.ncbi.nlm.nih.gov/pathogens/antimicrobial-resistance/AMRFinder/). This database contains over 4000 curated AMR protein sequences and are made available through HMM files.
...
This report contains all the resistance genes, found in the query genome, from [CARD](https://card.mcmaster.ca/), [NCBI-amr](https://www.ncbi.nlm.nih.gov/bioproject/PRJNA313047) and [Resfams](http://www.dantaslab.org/resfams) databases.
...
```{r, NCBI_conditional_block, echo=FALSE, results='asis', eval=ncbi_not_null, child='yes_ncbi.Rmd'}
```

```{r, NCBI_conditional_block_2, echo=FALSE, results='asis', eval=ncbi_null, child='no_ncbi.Rmd'}
```
