...
rgi_tsv <- try(read.delim(params$rgitool, header = TRUE), silent = TRUE) %>%
  separate(ORF_ID, into = c("Protein.Identifier", "Product"), sep = "\\s", extra = "merge") %>%
  select(Protein.Identifier, Product, Cut_Off, Best_Hit_ARO, ARO, Drug.Class,
...
Resistance genes or resistance determinants confer to the bacteria the ability to tolerate or resist antimicrobial molecules. Which means, these characteristics makes the bacteria less susceptible to its effects.
