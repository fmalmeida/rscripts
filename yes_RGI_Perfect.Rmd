...
```{r results='asis'}
## Name columns
colnames(rgi_perfect) <- c("Contig", "Start", "Stop", "Orientation", "Cut_Off", "Best_Hit_ARO", "ARO", "Drug Class", "Resistance Mechanism", "AMR Gene Family")
## Fix Contig Names
rgi_perfect$Contig <- gsub(pattern = " ", replacement = "", rgi_perfect$Contig)
rgi_perfect$Contig <- gsub(pattern = "_[0-9]*$", replacement = "", rgi_perfect$Contig)
## Create Coordinate
rgi_perfect$Coordinates <- paste(rgi_perfect$Contig, ":", rgi_perfect$Start, "-", rgi_perfect$Stop, sep = '')
## Separate rows
...
# Create List
string.list <- rgi_perfect %>% pull(8) %>% sort() %>% unique()
